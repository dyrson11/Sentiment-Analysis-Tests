...
# LSTM Layer
cell = tf.contrib.rnn.LSTMCell(dic_size, state_is_tuple=True)
outputs, final_rnn_state = tf.nn.dynamic_rnn(cell, x_input, dtype=tf.float32)
out_expanded = tf.expand_dims(outputs, -1)
...
fc_input = feature_maps(out_expanded, conv_def, dic_size, max_mat, out_conv_ch)
