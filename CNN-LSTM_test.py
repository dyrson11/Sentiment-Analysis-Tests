...
h_drop = tf.nn.dropout(fc_input, dropout_keep_prob)
...
fc_input = tf.expand_dims(h_drop, -1)
outputs, final_rnn_state = tf.nn.dynamic_rnn(cell, fc_input, dtype=tf.float32)
...
with tf.Session() as sess:
    ...
    for i in range(n_epoch):
        ...
        sess.run(train_function, feed_dict={x_input: x_train, y_output: y_train, dropout_keep_prob: 0.5})
        if i % 100 == 0:
            train_accuracy = accuracy.eval(feed_dict={x_input: x_train, y_output: y_train, dropout_keep_prob: 0.5})
            print('step %d, eficiencia del training %g' % (i, train_accuracy))

    # testing
    ef_test = accuracy.eval(feed_dict={x_input: x_test, y_output: y_test, dropout_keep_prob: 1})
